...
| `--mx-card-bg` | 输入框/卡片底 | 任意；背景开时被覆写为原色 @0.82 |
| `--mx-tabbar-bg` | 顶栏/底栏/pane 头 tabs | **必须设值**（见下方坑）；背景开时 @0.72 |
...
effect 里的两个 alpha 是全局手感：
- card-bg `0.82`：输入框/卡片（越大越实）
- tabbar-bg `0.72`：工具栏（稍透，玻璃感更强）
