/**
 * 中文字典 —— i18n 中文资源。
 *
 * namespace 按文件/功能模块切分,扁平点分 key 便于按文件定位。模板插值用
 * i18next 双花括号语法 `{{name}}`(escapeValue:false,React 已防 XSS)。
 * 品牌名(Claude/Codex/PowerShell 等)不入字典,i18next 缺失 key 回退到 key 本身即原样显示。
 */

const zh = {
  app: {
    loadingWorkspace: "正在加载工作区…",
  },
  common: {
    close: "关闭",
    loading: "加载中…",
    cancel: "取消",
    confirm: "确认",
    refresh: "刷新",
  },
  shell: {
    tab: {
      close: "关闭 tab",
      new: "新 tab",
    },
    pane: {
      split: "分屏(新 pane)",
      splitHorizontal: "横向",
      splitVertical: "纵向",
      close: "关闭 pane",
      noActiveTab: "无活动 tab",
    },
  },
  project: {
    select: "选择项目",
    none: "无项目。",
    add: "添加项目",
    addMenu: "打开项目 / 新窗口 / 历史项目",
    newProject: "新项目…",
    newWindow: "新窗口",
    recentSection: "历史项目",
    recentEmpty: "暂无历史项目",
    recentRemove: "从历史删除",
    delete: "删除项目",
    deleteConfirm: '删除项目 "{{name}}"?将关闭其所有 shell。',
    pin: "钉住(并排显示)",
    unpin: "取消钉住",
    detach: "在新窗口打开",
    focusDetached: "聚焦独立窗口",
    empty: "未选择项目。点右上 ✦ 打开项目 / 新窗口 / 历史项目。",
    emptyHint: "点右上 ✦ 打开项目",
    pinLimitReached: "已达钉住上限(10)",
    layout: {
      button: "并排布局",
      flow: "流向",
      flowRow: "横向",
      flowColumn: "纵向",
      groups: "分组",
      groupHint: "{{count}} 个项目",
    },
  },
  session: {
    all: "全部",
    current: "当前",
    empty: "暂无 Claude/Codex 会话记录",
    noMatch: "无匹配会话",
    loading: "加载中…",
    deleteGroup: "删除整组({{count}} 条)",
    deleteGroupConfirm: "删除整组 {{count}} 条?",
    noTitle: "(无标题){{id}}",
    unknownProject: "未知项目",
    unknownProjectPath: "(未知项目路径)",
    copyPath: "点击复制路径: {{cwd}}",
    copySessionId: "复制 session id",
    resume: "恢复会话",
    resumeById: "恢复",
    resumeSessionIdPlaceholder: "输入 session id 恢复…",
    resumeLast: "恢复上一次会话",
    resetSession: "重置会话",
    resumeLastEmpty: "暂无历史会话",
    messageCount: "{{count}} 条",
    startedAt: "开始",
    lastAt: "最近",
    deleteConfirm: "删除?",
    deleteRecord: "删除该会话记录",
    deleteSession: "删除会话",
    tipSelect: "从左侧选择一个会话查看详情",
    loadingMessages: "加载消息流…",
    noMessages: "无消息记录",
    copy: "点击复制",
    copyToolUse: "点击复制工具调用",
    copyToolResult: "点击复制工具结果",
    refresh: "刷新",
    searchPlaceholder: "搜索 标题 / 路径 / session id",
    filterCount: "过滤数 / 总数",
    role: {
      user: "用户",
      assistant: "助手",
    },
  },
  install: {
    selectAllCopy: "点击全选后 Ctrl+C 复制",
    notDetected: "未检测到 {{name}},请先安装:",
    notDetectedGroup: "同样未检测到 {{name}},建议一并安装:",
    afterInstallHint: "安装完成后,重新打开对应的窗口即可。",
    pathHint: '若已安装仍提示未装,请确认其所在目录已加入用户 PATH,并<strong style="font-weight:600;color:var(--mx-text)">重启本应用</strong>使新 PATH 生效。',
    fileYaziDep: {
      title: "file(yazi 依赖)",
      note: "yazi 用它做文件 MIME 类型检测,缺失会报错且预览不准",
    },
    lazygitNote: "终端里的 git TUI 客户端",
    yaziNote: "终端文件管理器",
    freshNote: "终端文本编辑器(零配置,类 VS Code 体验)",
  },
  preview: {
    selectFile: "从左侧选择文件预览",
    loading: "加载中…",
    binary: "二进制文件不可预览",
    truncated: "已截断:仅显示前 512 KB(完整 {{size}})",
    readError: "读取失败: {{error}}",
    editorLoading: "加载编辑器…",
  },
  probe: {
    edit: "编辑",
    readonly: "只读",
    unsaved: "未保存(5s 自动落盘,关闭即存)",
    tooLarge: "文件过大(>512KB),仅预览不支持编辑",
    mdPreview: "预览",
  },
  filetree: {
    emptyDir: "空目录",
    noProjectPath: "无项目路径",
    loading: "加载中…",
  },
  theme: {
    midnight: "午夜蓝",
    "one-dark": "One Dark",
  },
  settings: {
    title: "设置",
    theme: {
      title: "主题",
    },
    tab: {
      general: "通用",
      shortcuts: "快捷键",
      performance: "性能",
      about: "关于",
    },
    envCheck: {
      title: "环境体检",
      rerun: "重新检测",
      ok: "环境正常,包管理器不受杀软干扰",
      atRisk: "存在拦截风险:杀软实时扫描可能锁住 pnpm 等安装操作(EPERM/权限报错)",
      realTime: "Defender 实时防护开启(未排除项都会被扫描)",
      pnpmStore: "pnpm store 在排除列表(最关键,install 拦截主因)",
      node: "node.exe 在排除列表",
      txuy: "txuy-studio.exe 在排除列表",
      fixHint: "点复制,在管理员 PowerShell 粘贴执行一次即根治(应用自身不提权)。",
      copyFix: "复制修复命令",
      copied: "已复制 ✓",
    },
    performance: {
      hint: "实时性能检测(打开本页即采样,关闭即停)。长任务多/事件循环延迟高 = 界面卡顿来源;DOM 节点随会话持续增长 = 渲染负担;子进程数持续超出会话数 = 进程泄漏。报障时点「复制诊断报告」。",
      fps: "帧率 FPS",
      longTasks: "长任务 (>50ms)",
      loopLag: "事件循环延迟",
      domNodes: "DOM 节点数",
      jsHeap: "JS 堆内存",
      rustMemory: "Rust 进程内存",
      childProcesses: "子进程数",
      sessions: "会话数",
      copyReport: "复制诊断报告",
      copied: "已复制 ✓",
    },
    shortcut: {
      title: "快捷键",
    },
    language: {
      title: "语言",
      zh: "中文",
      en: "English",
    },
    fontSize: {
      title: "字体大小",
      hint: "终端 + 编辑器",
      decrease: "调小",
      increase: "调大",
      reset: "重置",
    },
    codexSandbox: {
      title: "Codex 沙箱",
      hint: "新建 Codex 会话的默认策略;已开会话不受影响,可在其状态栏单独切换",
    },
    bg: {
      title: "背景图",
      choose: "选择图片…",
      clear: "清除",
      blur: "模糊",
      dim: "暗化",
      hint: "开启后前景面板半透明,透出虚化背景图",
    },
    updater: {
      title: "应用更新",
      check: "检查更新",
      checking: "正在检查…",
      upToDate: "已是最新版本",
      install: "下载并安装",
      downloading: "下载中",
      installing: "正在安装…",
      installed: "安装完成",
      relaunch: "重启应用",
      error: "检查失败",
    },
  },
  about: {
    description: "面向 Windows 开发者的 AI CLI 终端工作台。并行管理 claude、codex、常规 shell、测试命令与项目任务。",
    version: "版本",
    viewOnGithub: "在 GitHub 查看",
  },
  statusbar: {
    settings: "设置",
    updateAvailable: "新版本可用",
    focusedProject: "聚焦项目",
    gitBranch: "git 分支:{{branch}}",
    memUsage: "内存占用 {{percent}}%",
    tipDone: "已完成,点击恢复显示;下一个 :00 / :30 自动轮换新提醒",
    tipActive: "每整点 / 半点(:00 / :30)轮换一次,点击标记为已完成",
    aiSessions: "AI 会话",
    aiRunning: "{{n}} 个运行中",
    aiWaiting: "{{n}} 个等待确认",
    aiRetrying: "{{n}} 个重试中",
    aiError: "{{n}} 个出错",
    aiBg: "{{n}} 个后台任务",
    aiIdle: "{{n}} 个空闲",
    contextUsed: "上下文已用 {{pct}}%",
    effort: "思考强度(effort)",
  },
  window: {
    minimize: "最小化",
    maximize: "最大化",
    restore: "向下还原",
    close: "关闭",
  },
  topbar: {
    backToMain: "回到主窗口",
    project: "项目",
  },
  sidebar: {
    openShellsByProject: "按项目浏览 shell",
    closePane: "关闭 pane",
    newShell: "新建 shell",
  },
  paneSurface: {
    noSession: "无 session:pane {{id}}",
  },
  // claudepane(自渲染 claude 对话面板):输入框 / 状态 / 工具卡片 / 错误提示文案。
  claudepane: {
    inputPlaceholder: "发送消息…(Enter 发送,Shift+Enter 换行)",
    slashHint: "输入消息,/ 调命令",
    busy: "Claude 正在响应…",
    thinking: "思考中…",
    runningTool: "正在执行 {{tool}}…",
    bgTasks: "后台任务 ×{{n}}…(可继续对话)",
    bgTaskDoneLabel: "后台任务完成",
    bgTaskFailedLabel: "后台任务失败",
    bgCountBadge: "后台 ×{{n}}",
    revealInFolder: "在文件夹中显示",
    mainTask: "主对话",
    mainWaiting: "等待确认",
    mainIdle: "空闲 · 可继续对话",
    bgTaskRunningSection: "后台任务 ×{{n}}",
    bgTaskNone: "无运行中的后台任务",
    bgTaskDoneSection: "最近完成",
    bgTaskStoppedLabel: "后台任务已停止",
    historyResumed: "已恢复会话历史({{n}} 条消息)",
    historyTruncated: "会话较大,仅回填最近部分",
    historyFailed: "历史回填失败,上下文已续接",
    killTaskTitle: "停止此任务",
    killTaskMsg: "[txuyStudio 系统操作] 请立即调用 TaskStop 工具停止后台任务 {{id}}({{desc}}),完成后仅回复 ok,不要做任何其他事。",
    killTaskQueuedShort: "待停止",
    killTaskQueuedMsg: "[txuyStudio 系统操作] 请依次调用 TaskStop 工具停止以下后台任务:{{tasks}},完成后仅回复 ok,不要做任何其他事。",
    thinkingExpand: "展开思考",
    interrupt: "中断",
    send: "发送",
    scrollToBottom: "回到底部",
    toolRunning: "执行中…",
    toolDone: "完成",
    permissionDenied: "需手动批准",
    terminated: "已终止",
    loading: "启动 Claude…",
    empty: "开始新的对话",
    notInstalled: "未检测到 Claude CLI,请先安装(推荐原生安装包):",
    installCmd: "irm https://claude.ai/install.ps1 | iex",
    retry: "重试",
    jsonResponse: "JSON 响应",
    planReady: "计划已就绪",
    approve: "批准执行",
    approveHint: "或拒绝后在输入框说明修改",
    reject: "拒绝/修改",
    planWaiting: "计划模式·等待你的确认",
    approveOnce: "批准本次",
    approvePersist: "批准且不再问",
    feedback: "反馈修改",
    approvalWaiting: "等待你的批准",
    approvalTitle: "需要你的批准",
    approveProceed: "已批准,请继续执行该操作",
    rejectMsg: "请不要执行 {{tool}},换一种方式",
    copy: "复制",
    copied: "已复制",
    notifTitle: "Claude 已完成",
    notifBody: "回复已就绪,点击查看",
    unsupported: "/{{cmd}} 在面板暂不支持,请在终端使用 claude CLI",
    unsupportedNoCwd: "未知工作目录,无法定位文件",
    helpTitle: "快捷键",
    costTitle: "本次会话用量",
    compactRunning: "正在压缩对话历史…",
    compactSummary: "压缩总结",
    compactFailed: "压缩失败:{{error}}",
    compactBoundary: "已压缩 {{pre}} → {{post}} tokens({{pct}}%)",
    compactBoundaryFallback: "已压缩对话历史",
    shellRunning: "运行中",
    shellDone: "完成",
    shellError: "失败({{code}})",
    shellInterrupted: "已中断",
    shellOutputTruncated: "输出过长,仅保留最后 {{n}} 行",
    retrying: "API 错误,正在重试({{n}}/{{max}})…",
    retryFailed: "重试 {{max}} 次仍失败{{error}}",
    modelCurrent: "当前",
    modelEmpty: "未读取到 claude 配置中的模型",
    modelCustomPlaceholder: "自定义 model 名",
    modelApply: "应用",
    rewindTitle: "回滚到某条消息",
    rewindHint: "选一条历史消息 → 清空会话 + 回填输入框,基于它重开",
  },
  // codexpane(自渲染 codex 对话面板):与 claudepane 平行,codex 特有文案
  // (无审批/压缩/重试文案;含 sandbox 拦截提示)。`!` 命令文案复用 claudepane.shell*。
  codexpane: {
    slashHint: "发送消息,/ 调命令",
    busy: "Codex 正在响应…",
    thinking: "思考中…",
    runningTool: "正在执行 {{tool}}…",
    interrupt: "中断",
    send: "发送",
    scrollToBottom: "回到底部",
    toolRunning: "运行中",
    toolDone: "完成",
    toolError: "失败",
    sandboxDenied: "被沙箱拦截",
    sandbox: {
      ro: "只读(写操作被沙箱拦截)",
      auto: "工作区可写(默认)",
      yolo: "完全访问(谨慎)",
    },
    terminated: "已终止",
    loading: "启动 Codex…",
    empty: "开始新的对话",
    notInstalled: "未检测到 Codex CLI,请先安装:",
    installCmd: "npm install -g @openai/codex",
    retry: "重试",
    jsonResponse: "JSON 响应",
    notifTitle: "Codex 已完成",
    notifBody: "回复已就绪,点击查看",
    unsupported: "/{{cmd}} 在面板暂不支持,请在终端使用 codex CLI",
    unsupportedNoCwd: "未知工作目录,无法定位文件",
    helpTitle: "快捷键",
    costTitle: "本次会话用量",
    modelCurrent: "当前",
    modelEmpty: "未读取到 codex 配置中的模型",
    reasoningTitle: "推理强度(reasoning)",
  },
  // 快捷键分组与描述(域常量 key 化后,SettingsModal 渲染时 t(item.title)/t(item.desc))
  shortcut: {
    split: {
      title: "分屏",
      down: "向下分屏(新 pane 单 PowerShell tab)",
      right: "向右分屏",
      close: "关闭焦点 pane",
    },
    focus: {
      title: "焦点",
      up: "焦点上移",
      down: "焦点下移",
      left: "焦点左移",
      right: "焦点右移",
    },
    tab: {
      title: "Tab",
      new: "新建 tab(复制当前活动 tab 类型)",
      close: "关闭当前 tab",
      next: "下一个 tab",
      prev: "上一个 tab",
    },
    project: {
      title: "项目",
      switchN: "切焦点到第 N 个项目(按可见顺序)",
    },
  },
  // Shell 类型 label(仅 sessionbrowser/filetree 需译,其余品牌名回退 key 本身)
  shellkind: {
    sessionbrowser: "会话列表",
    filetree: "探针",
    htmlpreview: "HTML 预览",
    notes: "笔记",
  },
  // 新建菜单分组标题(Shell/AI CLI 为英文,回退 key 本身)
  shellgroup: {
    tui: "TUI 工具",
    session: "会话",
    browse: "探针/笔记",
  },
  // HTML 预览面板(HtmlPreviewPane:编辑 + 沙箱预览 + 打开本地文件)
  htmlpreview: {
    source: "HTML 源码",
    clear: "清空",
    placeholder: "在此粘贴或编辑 HTML…",
    preview: "预览",
    pathPlaceholder: "输入本地 HTML 文件路径,回车打开…",
    browse: "选择文件",
    hideEditor: "隐藏编辑区",
    showEditor: "显示编辑区",
    loadedFrom: "已加载:",
    loadErrorBinary: "该文件不是可预览的文本 HTML",
    loadTruncated: "文件过大,仅预览前 512KB",
    loadErrorRead: "读取失败{{error}}",
  },
  // 笔记面板(NotesPane:Typora 式 md 编辑,左源码右实时预览,存项目根 .note 文件)
  notes: {
    newNote: "新建笔记",
    openNote: "打开笔记",
    source: "Markdown 源码",
    preview: "预览",
    saved: "已保存",
    unsaved: "未保存",
    saving: "保存中…",
    loadFailed: "读取失败",
    saveFailed: "保存失败",
    empty: "项目里还没有笔记,点 ✎ 新建一篇",
    emptyNotesDir: "暂无笔记文件",
    defaultTitle: "笔记",
    hideEditor: "隐藏编辑区",
    showEditor: "显示编辑区",
  },
  // 健康 tip(StatusBar HEALTH_TIPS 数据驱动,emoji 保留)
  tip: {
    water: { tip: "💧 起来喝杯水", done: "💧 已喝水" },
    lift: { tip: "🍑 该提肛了", done: "🍑 已提肛" },
  },} as const;

export default zh;
