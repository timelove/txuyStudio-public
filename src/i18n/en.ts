...
const en = {
  ...
  project: {
    ...
    none: "No projects.",
    add: "Add project",
    addMenu: "Open project / new window / recent projects",
    newProject: "New project…",
    newWindow: "New window",
    recentSection: "Recent projects",
  ...
  settings: {
    ...
    performance: {
      hint: "Live performance monitoring (sampling runs only while this page is open). Frequent long tasks / high event-loop lag = UI jank source; ever-growing DOM nodes = rendering load; child process count exceeding session count = process leak. Use \"Copy diagnostic report\" when reporting issues.",
      fps: "Frame rate (FPS)",
      longTasks: "Long tasks (>50ms)",
      loopLag: "Event-loop lag",
      domNodes: "DOM nodes",
      jsHeap: "JS heap",
      rustMemory: "Rust process memory",
    ...
    envCheck: {
      ...
      rerun: "Re-run",
      ok: "Environment OK — package managers are not blocked by antivirus",
      atRisk: "Interception risk: real-time AV scanning may lock pnpm-style installs (EPERM / permission errors)",
      realTime: "Defender real-time protection on (non-excluded items get scanned)",
      pnpmStore: "pnpm store in exclusion list (the critical one for installs)",
      node: "node.exe in exclusion list",
      txuy: "txuy-studio.exe in exclusion list",
      fixHint: "Copy, then paste once into an elevated PowerShell to fix permanently (the app never elevates itself).",
      copyFix: "Copy fix commands",
      copied: "Copied ✓",
    },
    fontSize: {
      title: "Font size",
  ...
  claudepane: {
    ...
    historyResumed: "Session history restored ({{n}} messages)",
    historyTruncated: "Large session: only the recent part is backfilled",
    historyFailed: "History backfill failed; context still resumed",
    ...
    notInstalled: "Claude CLI not detected. Please install it first (native installer recommended):",
