...
impl ClaudeRegistry {
    ...
    /// 应用退出时同步杀掉全部 claude 长进程(ExitRequested 回调,不能再 spawn_blocking)。
    /// 锁内 drain、锁外 taskkill /T 杀整棵进程树。
    pub fn kill_all_blocking(&self) {
        let drained: HashMap<String, HashMap<String, ClaudeSession>> =
        ...
        if count > 0 {
            log::info!("kill_all_blocking(claude): killed {count} process tree(s)");
        }
    }
}
