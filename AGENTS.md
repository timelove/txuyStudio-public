...
请尽量使用简体中文与我对话，并在回答时保持专业、简洁。

> 本文件与 `CLAUDE.md` 保持内容同步；如需修改项目指引，请编辑 `CLAUDE.md` 后重新同步。
...
当前阶段为 **MVP（已接入真实 PTY + 多项目 + 持久化）**：Tauri + Rust 后端、React + TypeScript + xterm.js 前端已跑通；真实 PTY（`portable-pty`/ConPTY）、多项目工作区、Windows Terminal 式分屏、布局/窗口持久化均已实现。`mockWorkspace` 退居为**纯浏览器兜底数据源**（`bun run dev` 无 Tauri 运行时时使用），桌面运行时以 Rust 后端 `hydrate_window` 持久化状态为主。实现进度详见 [`.work/plans/PROGRESS.md`](.work/plans/PROGRESS.md)。
...
```powershell
bun install                  # 安装依赖
bun run dev                  # 仅前端：Vite dev server (http://localhost:1420)（实际经 volta run --node 22 跑 vite,见下方说明）
bun run tauri dev            # 桌面开发：自动跑 bun run dev + 启动 Tauri 窗口（日常开发主入口）
bun run build                # 前端生产构建：tsc 类型检查 + vite build
bun run tauri build          # 完整打包：前端构建 + Rust release 编译 -> src-tauri/target/release/txuy-studio.exe
```

> **dev 脚本为何用 `volta run`**：`package.json` 的 `dev` 脚本是 `volta run --node 22.14.0 node node_modules/vite/bin/vite.js`（**非** `bunx --bun vite`）。原因：Bun runtime 跑 Vite optimizeDeps scanning **慢 40 倍**（dev 首屏卡在「正在加载工作区」超 1 分钟），而 bunshell（`bun run` 内）解析到的系统 node 是 16.15.1（Vite 8 需 20.19+,会崩 `CustomEvent is not defined`），故用 `volta run` 显式锁定 node 22 跑 vite。仍统一用 `bun run dev` / `bun run tauri dev` 入口；`package.json` 已 `volta.node: 22.14.0` pin。build 走 rollup 不受影响（保持 `bunx --bun vite build`）。详见 [dev-slow-boot design](.work/design/20260716-dev-slow-boot/dev-slow-boot-design-v1.md)。
...
环境注意：Rust 工具链在 `C:\Users\<user>\.cargo\bin`，当前 PowerShell 会话可能未自动加入 PATH，必要时先 `$env:Path += ";C:\Users\18061\.cargo\bin"`。Tauri bundling 阶段需联网下载 WiX，国内网络可能 `Peer disconnected`，属网络问题非代码错误。
...
- 两个实现：`MockTerminalTransport`（纯浏览器兜底，回放静态 transcript）与 `TauriPtyTransport`（真实 PTY：`start` 时先 `listen("pty-output")` 再 `invoke("spawn_pty")`，按 sessionId 路由输出，避免丢首批输出）。
- `TerminalPane.tsx` 只依赖 `TerminalTransport` 接口，**不直接耦合 Tauri/PTY API**。`AppShell` 按 paneId 池化 transport（pane 存活期复用，移除时 stop+delete，避免重渲染反复 spawn）。
...
- `domain/`：纯类型与领域模型。核心为 `paneTree.ts`（二叉分屏树的纯函数：split/close/listPanes/findPane/focusPane）、`appState.ts`（后端 `AppSnapshot` 的前端镜像）、`tauriPtyTransport.ts`（真实 PTY transport）、`projectDeriver.ts`（快照派生）。**新增类型放这里**，不要散落在组件文件。
- `mock/mockProjects.ts`、`mock/mockWorkspace.ts`：纯浏览器兜底数据源（仅 `bun run dev` 无 Tauri 运行时使用）。

### 后端结构（`src-tauri/`）

- `src/lib.rs`：`tauri::Builder` 入口。注册 `tauri-plugin-log`（写文件/stdout/webview）、`tauri-plugin-opener`、`tauri-plugin-dialog`；`setup` 中 `persistence::load` 从磁盘加载快照（失败回退空）并 `manage(AppState)`；`PtyRegistry` 另作 managed state。`invoke_handler` 注册全部命令。
- `pty/`：PTY 会话领域。`mod.rs` 定义 `PtySession`（writer/child/master）与全局 `PtyRegistry`（`Mutex<HashMap<sessionId, PtySession>>`）；`commands.rs` 实现 `spawn_pty`/`write_pty`/`resize_pty`/`kill_pty`，输出经全局 `pty-output` 事件回推。**当前 PTY 仍按全局 sessionId 归属，尚未按 projectId 隔离**（阶段 3 待办）。
- `state/`：多项目持久化。`mod.rs` 定义 `AppState`（`Mutex<AppSnapshot>`）、`ProjectRecord`、`PaneNode`、`WindowBounds`（与前端 `appState.ts` 类型镜像，camelCase 对齐）；`commands.rs` 实现 `hydrate_window`/`open_project`/`close_project`/`set_active_project`/`save_window_bounds`/`save_pane_tree`；`persistence.rs` 读写 `state.json`。
...
### Windows Terminal 式快捷键（在 `AppShell` 全局 keydown 注册）
